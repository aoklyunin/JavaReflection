package abstract_interface.work2;
...
public class ReflectionWork2 {
    public static void main(String[] args) {
        ...
        MyReflection.setClass(MyClass.class);

        MyReflection.checkImlementsInterface(Runnable.class);
        MyReflection.separate();
        MyReflection.getField("running").check(boolean.class,MODIFIER_PRIVATE);


        MyReflection.separate();
        MyReflection.checkMethod("start").check(void.class,MODIFIER_PUBLIC);
        MyReflection.separate();
        MyReflection.checkMethod("stop").check(void.class,MODIFIER_PUBLIC);
